...
def make_exe(dist):
    ...
    # python_config.run_mode = 'eval:import runpy; runpy.run_module("azure.cli")'

    # Run a Python module as __main__ when the interpreter starts.
    python_config.run_mode = 'module:azure.cli'
    ...
    #exe.add_python_resources(exe.pip_download(["azure-cli==2.12.1"]))
    ...
    # Install PyYAML first without libyaml, linking against libyaml is broken on MacOS
    for resource in exe.pip_install(["PyYAML==5.3.1", "--global-option=--without-libyaml"]):
        # Omit source code for all resources by default
        resource.add_source = False
        exe.add_python_resource(resource)

    # Install setuptools next, the pkg_resources module needs to be available at runtime
    for resource in exe.pip_install(["setuptools==49.2.1"]):
        # Omit source code for all resources by default
        resource.add_source = False
        exe.add_python_resource(resource)

    # Finally, install azure-cli and all its remaining deps
    for resource in exe.pip_install(["-r", "requirements.txt", "--no-deps"]):
        # Skip invoke's vendored Python 2.x version of pyyaml
        if "invoke.vendor.yaml2" in resource.name:
            continue

        # Load azure.cli modules from the filesystem and include source, otherwise the list of commands is blank
        # Load certifi from the filesystem and include source, it doesn't work if loaded in-memory
        if resource.name.startswith("azure.cli.") or resource.name.startswith("certifi"):
    ...
    # in a file.
    #exe.filter_from_files(files=["filter.txt"])
